    ...
    if (lastEnd < start) {
      for (let i = lastEnd; i < start; i += 1) {
        splits.push({
          ...
          linebreaksCount: tokens[i].linebreaksCount,
        });
      }
    }
    splits.push({
      ...
      text: tokens
        ...
        .map((t) => t.text)
        .join(" "),
      hasWhitespace: tokens.length > end - 1 ? tokens[end - 1].hasWhitespace : false,
      linebreaksCount: tokens.length > end - 1 ? tokens[end - 1].linebreaksCount : 0,
    });
    lastEnd = end;
  }
